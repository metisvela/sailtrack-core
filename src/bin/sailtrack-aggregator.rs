...
fn main() {
            ...
            if let Event::Incoming(Incoming::Publish(packet)) = notification {
                ...
                match topic.as_str() {
                    "sensor/imu0" => {
                        ...
                        on_message_imu(serde_json::from_slice(payload.as_ref()).unwrap(), &mut input_clone);
                    },
                    "sensor/gps0" => {
                        let payload = packet.payload.clone(); // Clone the payload for later use
                        on_message_gps(serde_json::from_slice(payload.as_ref()).unwrap(), &mut measure_clone);
                    },
                    _ => (),
                }
            }
    ...
    thread::spawn(move || {
        loop {
            ...
            if input_lock.new_input && measure_lock.new_measure {
    ...
    thread::spawn(move || loop {
        loop {
            ...
            let body_to_world_mtx = Matrix3::new(
                f32::cos(heading), f32::sin(heading), 0.0,
                -f32::sin(heading), f32::cos(heading), 0.0,
                0.0, 0.0, 1.0
            );
